...
void ClosestHitForward(inout RayIntersection rayIntersection : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
{
	...
	#ifdef N_F_TP_ON
		float4 _MainTex_var = RT_Tripl_Default(_MainTex, sampler_MainTex, input.positionWS.xyz, normalDirection); 
	#else
	...
	float RTD_GLO; 
	float3 RTD_GLO_COL;
	RT_GLO( fragInput.texCoord0.xy, fragInput.texCoord0.xy, halfDirection, normalDirection, viewDirection, fragInput.positionRWS.xyz, DirSpecDim, RTD_GLO, RTD_GLO_COL );
	...
	//=========//
		...
		if (LIGHTFEATUREFLAGS_PUNCTUAL)
		{
			...
			for (i = 0; i < i_en; ++i)
			{
				...
				float RTD_GLO; 
				float3 RTD_GLO_COL;
				RT_GLO( fragInput.texCoord0.xy, fragInput.texCoord0.xy, halfDirection, normalDirection, viewDirection, fragInput.positionRWS.xyz, Plight.specularDimmer, RTD_GLO, RTD_GLO_COL );
				float3 RTD_GLO_OTHERS = RTD_GLO;
				//==
				...
				if (NALC > 0)
				{
					...
					while (i <= last )
					{
						...
						if(IsMatchingLightLayer(Alight.lightLayers, builtinData.renderingLayers))
						{
							...
							float RTD_GLO; 
							float3 RTD_GLO_COL;
							RT_GLO( fragInput.texCoord0.xy, fragInput.texCoord0.xy, halfDirection, normalDirection, viewDirection, fragInput.positionRWS.xyz, Alight.specularDimmer, RTD_GLO, RTD_GLO_COL );
							//==
